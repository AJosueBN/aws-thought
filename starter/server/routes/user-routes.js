...
}); // closes the route for router.get(/users)

// Finds all thoughts from a user
    ...
    const params = {
     ...
     dynamodb.query(params, (err, data) => {
        if (err) {
          ...
          res.status(500).json(err); // an error occurred if get method fails
        } else {
...
router.post('/users', (req, res) => {
    ...
    dynamodb.put(params, (err, data) => {
        if (err) {
          ...
          res.status(500).json(err); // an error occurred if post method fails
        } else {
